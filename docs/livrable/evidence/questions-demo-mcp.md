...
|---:|---|---|---|---:|---|
| 1 | `check_stock` | `REF-8842` | ok | 86 ms | Le tool figé : une requête écrite d'avance, seul le paramètre varie. |
| 2 | `order_status` | `CMD-2025-0005` | ok | 10 ms | Format CMD-AAAA-NNNN vérifié avant toute exécution. |
| 3 | `ask_database` | `combien de commandes en avril 2026 ?` | ok | 1654 ms | Question libre : le modèle écrit le SQL, le validateur le contrôle. |
...
| 8 | `check_stock` | `nimportequoi` | refused · `INVALID_ARGUMENT` | 4 ms | Argument malformé : refusé par le service, donc journalisé. |
| 9 | `answer_question` | `quel est le délai d'un échange standard ?` | ok | 157 ms | Réponse documentaire citée, dans les seules collections du support. |
...
| 2 | `ask_database` | `quelle est la marge totale par catégorie ?` | ok | 1419 ms | Le périmètre suit le profil : la marge est autorisée ici. |
| 3 | `ask_database` | `quels sont les 3 clients qui ont le plus dépensé ?` | ok | 1458 ms | Classement : aucune des règles figées ne couvrait cette formulation. |
| 4 | `search_docs` | `remise commerciale négociation tarif` | ok | 132 ms | Les notes internes remontent ici, et seulement ici. |
...
| # | Tool | Argument à coller | Résultat observé | Durée | Ce que ça montre |
|---:|---|---|---|---:|---|
| 1 | `get_schema` | *(aucun)* | ok | 8 ms | Un IDE explore le périmètre SQL sans jamais lire une ligne métier. |
